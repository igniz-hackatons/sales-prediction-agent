...
export class WorkerFactory<TData = any, TResult = any> {
  ...
  constructor(
    ...
    this.worker = new Worker(
      ...
      async (job) => {
        try {
          ...
          logger.error(`[SERVER] [Worker:${this.name}] job failed:`, err);
          throw err;
        }
      },
      {
    ...
    this.worker.on('error', (err) => {
      logger.error(`[SERVER] [Worker:${this.name}] error:`, err);
    });

    this.worker.on('completed', (job) => {
      logger.info(`[SERVER] [Worker:${this.name}] completed job ${job.id}`);
    });

    this.worker.on('failed', (job, err) => {
      logger.warn(`[SERVER] [Worker:${this.name}] failed job ${job?.id}:`, err.message);
    });
  }
