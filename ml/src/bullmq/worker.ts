...
export class WorkerFactory<TData = any, TResult = any> {
    ...
    constructor(
        ...
        this.worker = new Worker(
            ...
            async (job) => {
                try {
                    ...
                    logger.error(`[ML] [Worker:${this.name}] job failed:`, err);
                    throw err;
                }
            },
            {
        ...
        this.worker.on("error", (err) => {
            logger.error(`[ML] [Worker:${this.name}] error:`, err);
        });

        this.worker.on("completed", (job) => {
            logger.info(`[ML] [Worker:${this.name}] completed job ${job.id}`);
        });

        this.worker.on("failed", (job, err) => {
            logger.warn(
                `[ML] [Worker:${this.name}] failed job ${job?.id}:`,
                err.message
            );
        });
    }
